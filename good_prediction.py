...
Forecast horizon:
  - The best model (highest Test R²) per substation is used to recursively
    forecast FORECAST_DAYS (default 14) days ahead, one day at a time —
    each predicted day feeds back into lag_1 / lag_7 / roll_mean_7 /
    roll_std_7 for the next day's prediction. Override with --forecast-days.

Model ranking (overall, across all substations):
  - model_ranking.csv / model_ranking.txt in OUTPUT_DIR rank all 6 models
    by mean Test R² across every substation, so you can pick a single
    "best overall" model for downstream work.
...
WINDOW        = 14
FORECAST_DAYS = 14   # how many days ahead to recursively forecast (was: 1 = "tomorrow")
TRAIN_END_YEAR = 2024
...
# ── Step 4: Context-based multi-day (recursive) prediction ───────────────────

def _next_day_holiday_info(df_full: pd.DataFrame, next_date: pd.Timestamp) -> tuple[str, int]:
    """Resolve the holiday type (and its encoded value) for a future date."""
    holiday_type_map = {"No Holiday": 0, "Public Holiday": 1, "Eid": 2, "Durga Puja": 3}
    next_holiday_type = "No Holiday"

    if "Holiday_type" in df_full.columns:
        exact = df_full[df_full["Date"] == next_date]
        if not exact.empty and pd.notna(exact["Holiday_type"].iloc[0]):
            next_holiday_type = exact["Holiday_type"].iloc[0]
        else:
        ...
        same_month = df_full[df_full["Date"].dt.month == next_date.month]
        festive_in_month = same_month[same_month["Holiday_type"].isin(FESTIVE_HOLIDAY_TYPES)]
        if len(festive_in_month) >= 3 and next_holiday_type == "No Holiday":
            next_holiday_type = festive_in_month["Holiday_type"].iloc[0]

    holiday_encoded = holiday_type_map.get(next_holiday_type, 0)
    return next_holiday_type, holiday_encoded
...
def get_context_window_for_next_day(
    ...
    """Single-day-ahead prediction (kept for backward compatibility / reuse
    as the first step of forecast_next_n_days)."""
    results = forecast_next_n_days(
        df_full, sub_df, load_col, feature_cols, scaler, model, n_features, n_days=1
    )
    r = results[0]
    return r["pred_mw"], r["window_desc"]
...
def forecast_next_n_days(
    ...
    """
    Recursively forecast `n_days` ahead (default 14, was previously just
    "tomorrow"). At each step the model predicts one day using the last
    WINDOW days of features; that prediction is then appended back onto the
    working frame so lag_1 / lag_7 / roll_mean_7 / roll_std_7 (and the
    sliding WINDOW itself) are correctly recomputed for the following day.

    Returns a list of dicts, one per forecasted day, in chronological order:
      {"date": Timestamp, "pred_mw": float, "window_desc": str}
    """
    season_map = {12: 0, 1: 0, 2: 0, 3: 1, 4: 1, 5: 1, 6: 2, 7: 2, 8: 2, 9: 2, 10: 3, 11: 3}
    weather_cols = [
        "Temp_Max_C", "Temp_Min_C", "Temp_Mean_C",
    ...
    work_df = sub_df.copy().reset_index(drop=True)
    results = []
    ...
    for _ in range(n_days):
        ...
        next_holiday_type, holiday_encoded = _next_day_holiday_info(df_full, next_date)
        is_festive = next_holiday_type in FESTIVE_HOLIDAY_TYPES

        # Future weather is unknown beyond the data horizon: persist the
        # most recent known values (same approach used elsewhere in this
        # script for missing weather data).
        new_row = {
            "Date": next_date,
            "Holiday_type_encoded": holiday_encoded,
            "Is_Weekend": int(next_dow in (4, 5)),
            "Season_Encoded": season_map[next_month],
            "Month_sin": np.sin(2 * np.pi * next_month / 12),
            "Month_cos": np.cos(2 * np.pi * next_month / 12),
            "DOW_sin":   np.sin(2 * np.pi * next_dow / 7),
            "DOW_cos":   np.cos(2 * np.pi * next_dow / 7),
        }
        for c in weather_cols:
            new_row[c] = last_row.get(c, 0.0)

        new_row["lag_1"] = last_row[load_col]
        new_row["lag_7"] = work_df[load_col].iloc[-7] if len(work_df) >= 7 else last_row[load_col]
        recent_7 = work_df[load_col].iloc[-7:]
        new_row["roll_mean_7"] = recent_7.mean()
        new_row["roll_std_7"]  = recent_7.std() if len(recent_7) > 1 else 0.0

        # Fill in any remaining expected feature columns from the last row.
        for c in feature_cols:
            if c not in new_row and c != load_col:
                new_row[c] = last_row.get(c, 0.0)

        last_window_raw = work_df[feature_cols].values[-WINDOW:].copy()

        if is_festive:
            hist = df_full[df_full["Holiday_type"] == next_holiday_type]
            festive_loads = hist[load_col].dropna()
            festive_mean  = festive_loads.mean() if len(festive_loads) > 0 else last_row[load_col]
        ...
        pred_mw = float(np.clip(pred_mw, 0, MAX_PLAUSIBLE_LOAD_MW))

        results.append({"date": next_date, "pred_mw": pred_mw, "window_desc": window_desc})
...
def run_substation(df: pd.DataFrame, substation: str, output_dir: Path) -> list[dict]:
    ...
    # ── Contextual N-day forecast (best test-R² model, recursive) ────────────
    best_idx = int(np.argmax([m["test_r2"] for m in all_metrics]))
    ...
    forecast_df = pd.DataFrame([
        {"substation": substation, "model": best_name, "date": r["date"].date(),
         "day_ahead": i + 1, "predicted_mw": round(r["pred_mw"], 2), "context": r["window_desc"]}
    ...
    context_lines = [
        ...
        f"({forecast[0]['date'].date()} → {forecast[-1]['date'].date()})",
        f"Best model: {best_name}  (Test R²={all_metrics[best_idx]['test_r2']:.4f})",
        f"7-day rolling mean (last known): {last_row.get('roll_mean_7', float('nan')):.1f} MW  "
        f"| Lag-1: {last_row.get('lag_1', float('nan')):.1f} MW",
        "",
    ]
    for i, r in enumerate(forecast):
        context_lines.append(
            f"  Day {i + 1:>2} ({r['date'].date()}): {r['pred_mw']:.1f} MW  — {r['window_desc']}"
        )
    context_text = "\n".join(context_lines)
    print(f"\n{context_text}")
    (output_dir / f"{substation.replace(' ', '_')}_{FORECAST_DAYS}day_forecast.txt").write_text(context_text, encoding="utf-8")
...
# ── Step 10: Overall model ranking (across ALL substations) ───────────────────

def save_model_ranking(summary: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    """
    Ranks the 6 models by their MEAN metrics across every substation (i.e.
    overall performance, not per-substation), and saves the ranking to both
    a CSV and a human-readable text file so it can be reused by later work
    (e.g. picking the single best model to deploy).

    Saves:
      model_ranking.csv  — rank, model, mean train/test R², RMSE, MAE
      model_ranking.txt  — same, formatted for quick reading
    """
    grouped = (
    ...
    lines = [
        "=" * 90,
        "OVERALL MODEL RANKING — mean metrics across ALL substations "
        f"(sorted by Test R², n_substations={summary['substation'].nunique()})",
        "=" * 90,
    ]
    for _, row in grouped.iterrows():
        lines.append(
            f"  #{int(row['rank'])}  {row['model']:<22} "
            f"Test R²={row['test_r2']:.4f}  Train R²={row['train_r2']:.4f}  "
            f"Test RMSE={row['test_rmse']:.2f}  Test MAE={row['test_mae']:.2f}"
        )
    lines.append("")
    lines.append(
        f"BEST OVERALL MODEL: {grouped.iloc[0]['model']} "
        f"(mean Test R²={grouped.iloc[0]['test_r2']:.4f} across all substations)"
    )
    text = "\n".join(lines)
    (output_dir / "model_ranking.txt").write_text(text)

    print("\n" + text)
    print(f"\nSaved model_ranking.csv and model_ranking.txt to {output_dir.resolve()}")
...
def main():
    ...
    parser.add_argument("--forecast-days", type=int, default=FORECAST_DAYS,
                         help="How many days ahead to forecast (default: 14).")
